...
public class GameMainManager : MonoBehaviour
{
    ...
    //エンドカウント
    private float _e_count;
    //エンドカウントベース
    public float[] _e_count_b = new float[2];

    //トータルステージ数
    public int _t_stage=2;
    ...
    void Awake()
    {
        ...
        _ClearManager = _Clear.GetComponent<ClearManager>();
    ...
    void FixedUpdate()
    {
        if (_st==1)
        {
            ...
            if (_timer>=_e_time)
            {
                ...
                GameManager._success_per = GameManager._success_c / _e_count * 100;
                _Clear.SetActive(true);
                if (GameManager._success_per>=70)
                {
                    _ClearManager.ActiveSet(1);
                }
                else
                {
                    _ClearManager.ActiveSet(2);
                }
            }
        }
    }
