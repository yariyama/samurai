...
public class PlayerManager : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        _Mirror = transform.Find("Mirror").gameObject;
        _MirrorManager = _Mirror.GetComponent<MirrorManager>();
    ...
    void Update()
    {
        ...
        if (Input.GetKey("z"))
        {
            if (!_push_st2) { 
                _push_st2 = true;
                if (_MirrorManager._st == 0)
                {
                    _MirrorManager.ActiveSet();
                }
                else if (_MirrorManager._st == 1)
                {
                    _MirrorManager.DelSet();
                }
            }
        }
        else
