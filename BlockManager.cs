...
public class BlockManager : MonoBehaviour
{
    ...
    //アザーブロックスクリプト
    private BlockManager _OtherBlockManager;
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            _OtherBlockManager = other.GetComponent<BlockManager>();
            _other_psition = other.gameObject.transform.localPosition;
            _position = transform.localPosition;
            //★
            if (_OtherBlockManager._tp == 3)
            {
                ...
                {
                    _position.y = _other_psition.y + 0.55f;
                }
            }
            else
            {
                if (_tp == 3)
                {
                    _position.y = _other_psition.y + 1.5f;
                }
                else
                {
                    _position.y = _other_psition.y + 1f;
    ...
    public void ActiveSet(int _no)
    {
        ...
        else if (_no == 2)
        {
            ...
            //★
            GameManager._block_angle.Add(_angle);
        }
